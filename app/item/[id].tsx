...
export default function ItemDetails() {
  ...
  const handleDeactivate = () => {
    showConfirm(
      "Confirmation",
      "Voulez-vous vraiment désactiver cet item ?",
      async () => {
        try {
          ...
          showAlert("Succès", "Item désactivé");

          setItem({ ...item, active: false });

        } catch (error: any) {
          console.log("Deactivate error:", error?.response?.data);
          showAlert("Erreur", "Impossible de désactiver");
        } finally {
  ...
  return (
    ...
    <ScrollView style={styles.container}>
      ...
      {/* ======== BOUTON DÉSACTIVER (Owner uniquement) ======== */}
      {isOwner && item.active !== false && (
        <Pressable
        ...
        >
          <Text style={styles.buttonText}>
            {deactivateLoading ? "Désactivation..." : "Désactiver l'item"}
          </Text>
        </Pressable>
      )}
